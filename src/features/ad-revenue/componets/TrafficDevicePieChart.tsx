...
export function DevicePieChart() {
  ...
  useEffect(() => {
    ...
    if (period === 'month') {
      ...
      daysAgo30.setDate(now.getDate() - 240);
    ...
    for (const item of filtered) {
      const rawDevice = item.device?.toLowerCase() || 'unknown';
      const translated = DEVICE_TRANSLATIONS[rawDevice] || rawDevice;
  ...
  const totalVisits = chartData.reduce((acc, cur) => acc + cur.count, 0);
    ...
    <Card className='flex w-full flex-col'>
      ...
      <CardFooter className='flex-col gap-2 text-sm'>
        <div className='flex items-center gap-2 leading-none font-medium'>
          Всего посещений: {totalVisits} <TrendingUp className='h-4 w-4' />
        </div>
