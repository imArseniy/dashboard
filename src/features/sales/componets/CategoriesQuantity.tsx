...
export const description = 'Топ категорий по количеству продаж';
  ...
  React.useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const productsData: { id: string; category: string }[] =
          await productsRes.json();

        const productMap = new Map(
          productsData.map((p) => [p.id, p.category || 'Другие'])
        );
        ...
        salesData.forEach((sale) => {
          ...
          const rawCategory = productMap.get(sale.product_id) || 'Другие';
          const category =
            categoryNames[rawCategory] || rawCategory || 'Другие';
          const quantity = Number(sale.quantity) || 0;
          categoryMap.set(
            category,
            (categoryMap.get(category) || 0) + quantity
          );
        });
        ...
        const otherQuantity = data
          .slice(5)
          .reduce((sum, d) => sum + d.quantity, 0);
        if (otherQuantity > 0)
          topData.push({ category: 'Другие', quantity: otherQuantity });
  ...
  return (
    <Card className='flex w-full flex-col'>
      <CardHeader className='flex items-center justify-between pb-0'>
        <div>
          <CardTitle>Категории по количеству продаж</CardTitle>
          <CardDescription>Все категории по количеству продаж</CardDescription>
        </div>
      ...
      <CardFooter className='flex-col gap-2 text-sm'>
        <div className='text-muted-foreground leading-none'>
          Категории по количеству продаж
        </div>
      </CardFooter>
    </Card>
  );
